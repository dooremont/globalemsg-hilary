...
// Todo  return callback({'code': 401, 'msg': 'Only Admin users can create globale message'});
 ...
 log().info("Create gms object :",gms);
    ...
    Cassandra.runQuery(cql, parameters, function(err) {
        ...
        if (gms.level === 1){
            var mail = user.email
            log().info("Envoie de mail :");
            log().info("Envoie de mail  à :",mail);
            EmailAPI.sendEmail('oae-globalemsg', 'gms', user, null, null, function(err, message) {
                                        if (err){
                                                log().info("Envoie de mail  à :",user.mail);
                                                log().info("Erreur ",err,message);
                                        }else{
                                                log().info("Envoie de mail  à :",user.mail);
                                                log().info("Ok ",err,message);
                                        }});
   ...
   // var cql = 'SELECT * FROM "Gms" WHERE "userId" = ?';
    ...
    Cassandra.runQuery(cql, parameters, function(err, rows) {
        ...
        rows.forEach(function(row) {
            ...
            for (id in gms){
                log().info("Cassandra id :",id," ",gms[id]);
            }
            gmss.push(gms);
        });
...
module.showGms = function(ctx,callback){
    ...
    Cassandra.runQuery(cql, parameters, function(err, rows) {
        if (err) {
            log().info("Cassandra SELECT * FROM Gms WHERE created :"+err);
            return callback(err);
        }
        var ListgmsAviables = [];
        rows.forEach(function(row) {
