...
var mailUsersFromTenant = function(tenantAlias,data ,callback) {
var data =data;
    ...
    PrincipalsDAO.iterateAll(['principalId', 'tenantAlias', 'displayName', 'publicAlias', 'email'], 50, handlePrincipals, function(err) {
        if (err) {
...
var mailUsers = function(users,gms,callback) {
    ...
    log().debug("Email users",users);
    log().debug("Email gms",gms);
    log().debug("Email send at user",user);
   ...
   // EmailAPI.sendEmail('oae-globalemsg', 'gms', user, gms, null, function(err, message) {
    EmailAPI.sendEmail('oae-globalemsg', 'gms', user, gms, null, function(err,message) {
         ...
         log().debug("Email sans boulce",user);
    //     mailUsers(users, gms, callback);
     });
    // mailUsers(users, gms, callback);
};
...
log().debug("Octx.tenant().alias,gms ",ctx.tenant().alias,gms);
        mailUsersFromTenant(ctx.tenant().alias,gms);
        }
