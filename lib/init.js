...
module.exports = function(config, callback) {
    ...
    Cassandra.createColumnFamily('Gms', 'CREATE TABLE "Gms" ("userId" text, "title" text, "description" text, "date_start" bigint, "date_end" bigint, "send_mail" int, "level" int, "Alias_tenant" text,"created" bigint, PRIMARY KEY("userId", "created", "date_end", "date_start", "Alias_tenant"))', callback);
    log().info("Creation de la base GMS table Gms",callback);
    //Cassandra.createColumnFamily('Gms_user', 'CREATE TABLE "Gms_user" ("PrincipalId" text, "userId" text, "title" text, "date_created" bigint, PRIMARY KEY("PrincipalId"))', callback);
    log().info("Creation de la base GMS table Gms_user",callback);
};
